...
pub async fn open_file() -> Result<PathBuf, OpenError> {
    ...
    let default_dir = match paths::default_save_game_dir().and_then(|p| p.to_str().map(|s| s.to_owned())) {
        Some(d) => d,
        None => panic!("No home directory set. On what can we fallback ?")
    };

    let result: Result<Option<String>, tokio::task::JoinError> = tokio::task::spawn_blocking(move || {
        tinyfiledialogs::open_file_dialog("Choose a file to open", &default_dir, Some((&["*.zks"], "PF Save Files")))
    }).await;
    ...
    */
    ...
    mod win{
        ...
        pub fn save_game() -> Option<PathBuf> {
            ...
            dirs::data_local_dir().map(|h| h.join("..\\LocalLow\\Owlcat Games\\Pathfinder Kingmaker\\Saved Games"))
        }
    }
    #[cfg(target_os = "windows")]
    use win as os;
